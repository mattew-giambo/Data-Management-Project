...
# Helper functions for temporal attributes (Lustro and Pandemic Period)
def get_lustro(year):
    if year <= 2014:
        return "2010-2014"
    elif year <= 2019:
...
ev_wide["lustro"] = ev_wide["year"].apply(get_lustro)
ev_wide["pandemic_period"] = ev_wide["year"].apply(get_pandemic_period)
...
# Add temporal attributes
co2_clean["lustro"] = co2_clean["year"].apply(get_lustro)
co2_clean["pandemic_period"] = co2_clean["year"].apply(get_pandemic_period)
...
# Add temporal attributes
energy_clean["lustro"] = energy_clean["year"].apply(get_lustro)
energy_clean["pandemic_period"] = energy_clean["year"].apply(get_pandemic_period)
